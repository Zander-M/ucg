...
AlignedBox3d bbox_triangle(const Vector3d &a, const Vector3d &b,
                           const Vector3d &c) {
    AlignedBox3d box;
    box.extend(a);
    box.extend(b);
...
AABBTree::AABBTree(const MatrixXd &V, const MatrixXi &F) {
    ...
    std::vector<struct centroid *> *v_centroids =
        new std::vector<struct centroid *>;
    ...
    while (q_centroid.size() > 0) {
        ...
        if (list->size() == 1) {
            ...
            int mid = list->size() / 2;
            std::vector<struct centroid *> *left =
                new std::vector<struct centroid *>;
            std::vector<struct centroid *> *right =
                new std::vector<struct centroid *>;
            for (int i = 0; i < mid; ++i) {
...
bool Parallelogram::intersect(const Ray &ray, Intersection &hit) {
    ...
    p_gram << u, v;
    Vector2d portion =
        p_gram.colPivHouseholderQr().solve(intersection - this->origin);
...
bool intersect_triangle(const Ray &ray, const Vector3d &a, const Vector3d &b,
                        const Vector3d &c, Intersection &hit) {
    // TODO (Assignment 3)
    //
...
////////////////////////////////////////////////////////////////////////////////
// Method 2: BVH accelerate 
////////////////////////////////////////////////////////////////////////////////

bool Mesh::intersect(const Ray &ray, Intersection &closest_hit) {
    // Method (2): Traverse the BVH tree and test the intersection with
    // a triangles at the leaf nodes that intersects the input ray.
    Intersection hit;
    double min_param = 1000;
    bool intersect = false;
    AABBTree tree = bvh;
    ...
    while (q_nodes.size() > 0) {
        ...
        q_nodes.pop();
        if (curr.triangle != -1) {
            if (intersect_triangle(ray, vertices.row(facets(curr.triangle, 0)),
                                   vertices.row(facets(curr.triangle, 1)),
                                   vertices.row(facets(curr.triangle, 2)),
                                   hit)) {
                if (hit.ray_param < min_param) {
                    closest_hit = hit;
                    min_param = hit.ray_param;
...
Vector3d ray_color(const Scene &scene, const Ray &ray, const Object &obj,
    ...
    Vector3d ambient_color =
        obj.material.ambient_color.array() * scene.ambient_light.array();
    ...
    for (const Light &light : scene.lights) {
        ...
        specular = mat.specular_color *
                   std::max(pow(phong_exp, mat.specular_exponent), 0.0);

        // Attenuate lights according to the squared distance to the
        // lights
        Vector3d D = light.position - hit.position;
        lights_color += (diffuse + specular).cwiseProduct(light.intensity) /
                        D.squaredNorm();
    }
...
Object *find_nearest_object(const Scene &scene, const Ray &ray,
    ...
    if (closest_index < 0) {
        ...
        // Return a pointer to the hit object. Don't forget to set
        // 'closest_hit' accordingly!
...
bool is_light_visible(const Scene &scene, const Ray &ray, const Light &light) {
    ...
    for (const ObjectPtr &h_obj : scene.objects) {
        if (h_obj->intersect(ray, hit_obj) && hit_obj.ray_param > eps &&
            hit_obj.ray_param < light_param) {
            return false;
        }
    }
    return true;
...
Vector3d shoot_ray(const Scene &scene, const Ray &ray, int max_bounce) {
    ...
    if (Object *obj = find_nearest_object(scene, ray, hit)) {
        // 'obj' is not null and points to the object of the scene hit
        // by the ray
...
void render_scene(const Scene &scene) {
    ...
    // The sensor grid is at a distance 'focal_length' from the camera
    // center, and covers an viewing angle given by 'field_of_view'.
    double aspect_ratio = double(w) / double(h);
    double scale_y =
        scene.camera.focal_length * tan(scene.camera.field_of_view / 2) * 2;
    ...
    for (unsigned i = 0; i < w; ++i) {
        ...
        for (unsigned j = 0; j < h; ++j) {
            for (unsigned k = 0; k < iter; ++k) {
                Vector3d shift = grid_origin + (i + 0.5) * x_displacement +
                ...
                double origin_y_offset =
                    ...
                    sqrt(1 - origin_x_offset * origin_x_offset);
                ...
                if (scene.camera.is_perspective) {
                    ...
                    ray.origin = scene.camera.position +
                                 offset * scene.camera.lens_radius;
                    ray.direction = shift - ray.origin;
                } else {
                    // Orthographic camera
                    ray.origin =
                        scene.camera.position + Vector3d(shift[0], shift[1], 0);
...
Scene load_scene(const std::string &filename) {
    ...
    for (const auto &entry : data["Objects"]) {
        ...
        if (entry["Type"] == "Sphere") {
            ...
            std::string filename =
                std::string(DATA_DIR) + entry["Path"].get<std::string>();
